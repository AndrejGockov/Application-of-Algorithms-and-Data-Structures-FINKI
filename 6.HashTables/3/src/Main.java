...
public class Main {
    public static void main(String[] args) {
        ...
        CBHT<String, Person>earliestUsers = new CBHT<>(n);
        ...
        for(int i = 0; i < n; i++){
            ...
            SLLNode<MapEntry<String, Integer>>customer = customersInCity.search(City);
            if(customer == null)
                customersInCity.insert(City, 1);
            else
                customersInCity.insert(City, customer.element.value + 1);

            SLLNode<MapEntry<String, Person>>earliest = earliestUsers.search(City);
            if(earliest == null
            || time.compareTo(earliest.element.value.time) < 0)
                earliestUsers.insert(City, person);
        }
        ...
        for(int i = 0; i < m; i++){
            ...
            System.out.println(customer.element.value);
