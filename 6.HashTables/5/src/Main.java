...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        int n = input.nextInt();
        CBHT<String, Integer>customersInNetwork = new CBHT<>(n);
        CBHT<String, Person>highestPayInNetwork = new CBHT<>(n);
        ...
        for(int i = 0; i < n; i++){
            ...
            SLLNode<MapEntry<String, Integer>>customers = customersInNetwork.search(sliceIp(ipAdress));
            if(customers == null){
                customersInNetwork.insert(sliceIp(ipAdress), 1);
            }else{
                customersInNetwork.insert(sliceIp(ipAdress), customers.element.value + 1);
            }

            SLLNode<MapEntry<String, Person>>pay = highestPayInNetwork.search(sliceIp(ipAdress));
            if(pay == null
                    || pay.element.value.paid < paid){
                highestPayInNetwork.insert(sliceIp(ipAdress), person);
            }
        }
