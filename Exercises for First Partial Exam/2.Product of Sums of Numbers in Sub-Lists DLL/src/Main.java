...
public class Main {
    ...
    public static long findMagicNumber(DLL<DLL<Integer>> list) {
        ...
        long ans = 1;
        int n = list.length();
        // Convert it from DLL<DLL> to DLLNode<DLL> to enable itteration
        DLLNode<DLL<Integer>>tmp = list.getFirst();

        // Itterate through every column and row
        while(tmp != null){
            DLLNode<Integer>row = tmp.element.getFirst();
            long sum = 0;
            ...
            // If the sum isn't equal to 0 we multiply
            if(sum > 0)
                ans *= sum;
