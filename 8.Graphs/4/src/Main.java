...
//        Излез:
//        2
//        (Објаснување: Ако не може да се користат патиштата Paris-Berlin и London-Brussels, градовите ќе се поделат во две групи: {London, Paris, Brussels, Amsterdam} и {Berlin, Warsaw})
//
//        /
//
...
//        Output:
//        2
//        (Explanation: If the roads Paris-Berlin and London-Brussels can't be used, the cities will be split into two groups: {London, Paris, Brussels, Amsterdam} and {Berlin, Warsaw}.)
...
class AdjacencyListGraph<T> {
    ...
    public void DFS(T startVertex, Set<T> visited) {
...
//        System.out.print(vertex + " ");
    ...
    public int countRouter(){
        ...
        for(T router : adjacencyList.keySet()){
...
public class Main {
    public static void main(String[] args) {
        ...
        for(int i = 0; i < n; i++){
            cities.addEdge(input.next(), input.next());
        }

        int m = input.nextInt();

        for(int i = 0; i < m; i++){
            cities.removeVertex(input.nextLine());
        }

        System.out.println(cities.countRouter());
    }
}